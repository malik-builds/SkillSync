                ...
                <div className="p-5 space-y-4">
                    ...
                    <div className="bg-red-50 p-3 rounded-lg border border-red-100">
                        <p className="text-[11px] text-red-700 font-semibold mb-2">
                            Warning: This action cannot be undone. University profile settings, interventions, and dashboard workspace data will be deleted immediately.
                        </p>
                    </div>
                    <div>
...
function AccountTab() {
    ...
    const hasUnsavedChanges = useMemo(() => {
        ...
        const keys: Array<keyof UniversityAccountSettings> = ["personalName", "personalRole", "personalPhone", "faculty"];
        const current: Partial<UniversityAccountSettings> = { ...account, ...formData };
        return keys.some((key) => {
            const nextVal = String(current[key] ?? "").trim();
            const baseVal = String(account[key] ?? "").trim();
