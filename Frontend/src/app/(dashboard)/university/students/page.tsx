...
export default function StudentAnalyticsPage() {
    ...
    const { data: scoreData } = useApi<ScoreDistributionBin[]>(
        () => getScoreDistribution(selectedProgramme === "All Programmes" ? undefined : selectedProgramme),
        [selectedProgramme]
    );
    ...
    const PROFILE_COMPLETED_STUDENTS = stats?.profileCompletedStudents ?? 0;
    const GITHUB_CONNECTED_STUDENTS = stats?.githubConnectedStudents ?? 0;
    const CV_UPLOADED_STUDENTS = stats?.cvUploadedStudents ?? 0;
        ...
        <div className="space-y-6 pb-20">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-4">
                ...
                <KpiCard
                    ...
                    value={`${PROFILE_COMPLETED_STUDENTS}`}
                    sub=""
                    icon={FileText}
                />
                <KpiCard
                    label="GitHub Connected"
                    value={`${GITHUB_CONNECTED_STUDENTS}`}
                    sub=""
                    icon={Github}
                />
                <KpiCard
                    label="CV Uploaded"
                    value={`${CV_UPLOADED_STUDENTS}`}
                    sub=""
                    icon={FileText}
                />
            </div>
